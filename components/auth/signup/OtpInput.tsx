...
export function OTPInputComponent({ updateStep, email }: Record<string, any>) {
  ...
  const editEmail = () => {
    window.location.replace("/auth/sign-up-email");
  };

  const checkVerification = async (email: string, otp: string) => {
    setIsLoading(true);
  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        console.log("OTP c", { email, verificationCode });
        await checkVerification(email, verificationCode);
      }}
      className="formClass"
    >
      <div>

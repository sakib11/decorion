...
export function PasswordInput({ email }: any) {
  ...
  type dataKey = "email" | "password";

  const savePassword = async (data: Record<dataKey, string>) => {
    setIsLoading(true);
  ...
  useEffect(() => {
    if (!passwordError && !confirmPasswordIntialError) {
      setShowSubmit(true);
    } else {
      setShowSubmit(false);
    }
  }, [passwordError, confirmPasswordIntialError]);
  ...
  return (
    ...
    >
      ...
      {passwordError && (
        <div className="errormsg rpassworderror errormsgcolor">
          Must be 8 digit alphanumeric
        </div>
      )}
      ...
      {confirmPasswordError && (
        <div className="errormsg rcpassworderror errormsgcolor">

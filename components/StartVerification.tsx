...
export function StartVerification({ onSubmit }: Record<string, any>) {
  ...
  useEffect(() => {
    ...
    if (removePlus.length === 13 && removePlus.startsWith("01") === true) {
      setIsValid(false);
      return;
    }

    if (
      [11, 13].includes(removePlus.length) === true &&
      (removePlus.startsWith("8801") === true ||
        removePlus.startsWith("01") === true)
    ) {
      setIsValid(true);
    }
  }, [phone]);
